...
module.exports.queryPatientName = (req, res) => {
  ...
  .catch((error) => res.status(500).json({error: error.message || "Error in querying by name"}))
}
...
module.exports.createPatient = (req, res) => {
  ...
  if (isNaN(age) || age < 1 || age > 130) {
    return res.status(400).json({ error: "Age can only be a number between 1 and 130." });
  }
...
module.exports.deletePatient = (req, res) => {
  ...
  .catch((err) => res.status(500).json({error: err.message || "Error in deleting patient"}))
